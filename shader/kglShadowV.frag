...
void main()
{
	...
	if ((x < x_center && x_texel < x) || (x > x_center && x_texel >= x)) {
		...
		int y_start = wall && y1 >= y_center ? y1 : int(clamp(y_start_raw + 0.5, 0., float(h)));
		int y_end = wall && y2 >= y_center ? y2 - 1 : int(clamp(y_end_raw + 0.5, -1., y2 < y_center ? float(y2) - 1. : float(h) - 1.)); // This bounds check is incorrect but is consistent with the original shader
